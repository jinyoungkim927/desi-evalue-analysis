...
\end{align}
With 2 extra parameters and $\Delta\chi^2 = 11.9$, the proper frequentist calculation uses the $\chi^2(2)$ distribution: $p = 1 - F_{\chi^2}(11.9;\, 2) = e^{-5.95} \approx 0.0026$, which corresponds to $\approx 2.8\sigma$ (two-sided). DESI correctly performs this conversion (see their equation~22, citing Wilks' theorem). The naive formula $\sqrt{\Delta\chi^2} \approx 3.5\sigma$ applies only for $k=1$ (see Section~\ref{sec:sigma_correct}).
...
The conversion from $\Delta\chi^2$ to ``number of sigma'' depends on the number of extra parameters $k$. We review the proper procedure here, since the $k=1$ formula sometimes appears in informal discussions.
...
\textbf{$k=1$ formula} (not applicable here): $\sigma = \sqrt{\Delta\chi^2} = \sqrt{11.9} \approx 3.45\sigma$.

This is valid only when there is 1 extra parameter. It treats $\Delta\chi^2$ as if it were the square of a single standard normal variable. DESI does not use this formula --- they correctly use the $\chi^2(2)$ CDF (their equation~22).
...
\begin{tabular}{ccc}
\toprule
$\Delta\chi^2$ & $\sigma$ ($k=1$) & $\sigma$ ($k=2$) \\
\midrule
6.0 & 2.4 & 1.8 \\
9.2 & 3.0 & 2.3 \\
11.9 & 3.5 & 2.8 \\
...
\begin{enumerate}
    ...
    \item \textbf{The frequentist BAO-only significance is $\approx 2.8$--$3.0\sigma$.} DESI correctly uses the $\chi^2(2)$ distribution and Wilks' theorem for this conversion (their equation~22). We note that the naive formula $\sqrt{\Delta\chi^2}$, which is valid only for $k=1$, would give $\approx 3.5\sigma$ --- this formula sometimes appears in informal discussions but was not used by DESI.
...
\textit{Code and data: \url{https://github.com/jinyoungkim927/desi-evalue-paper}}\\
