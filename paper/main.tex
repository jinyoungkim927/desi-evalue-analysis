...
All data-split e-values yield $E < 3$, substantially below the same-data maximized likelihood ratio, though the power caveats discussed above apply to all splits.

\paragraph{Uniform mixture e-values.} For completeness, we note that a uniform mixture e-value---which averages the likelihood ratio over a grid of $(w_0, w_a)$ values---yields $E \approx 15$ with the default prior range, and varies from approximately 15 to 97 as the prior range is narrowed. On conventional scales, $E = 15$ ($\ln E \approx 2.7$) constitutes moderate-to-strong evidence. The variation from 15 to 97 reflects the Occam razor operating as expected: a narrower prior that concentrates mass near the MLE produces larger e-values because it ``wastes'' less probability on parameter values far from the data. This is a standard feature of Bayesian and mixture-based tests, not a deficiency. The uniform mixture e-value is a valid e-value (unlike the maximized likelihood ratio) and should be considered alongside the data-split result when assessing the overall strength of evidence.
...
All three approaches---Bayesian model comparison, tension metrics, and e-value analysis---converge on the conclusion that inter-dataset tensions are a central issue. They differ, however, on the strength of the within-DESI evidence: the Bayes factor and data-split e-value suggest weak or no evidence, while the uniform mixture e-value suggests moderate evidence. This spread itself is informative, indicating that the answer depends on how one handles the composite alternative hypothesis.
...
E-value (data-split) & $E = 1.4$ & Inconclusive & Out-of-sample test \\
\bottomrule
\end{tabular}
\end{table}

\noindent AIC includes no Occam factor (it penalizes only the number of parameters), so it tends to favor more complex models. BIC approximately includes an Occam factor through $k\ln n$, but the result depends on what counts as a ``data point.'' The Bayes factor includes the full Occam penalty through the prior volume: with broad priors over $(w_0, w_a)$, much of the prior mass falls on parameter values that fit the data poorly, penalizing $w_0w_a$CDM. The e-value results span this range depending on methodology: the uniform mixture e-value ($E \approx 15$) is intermediate, while the data-split e-value ($E = 1.4$) tests a different question entirely (out-of-sample prediction).

These different answers are not contradictory---they reflect different questions. AIC asks which model will predict future data better on average (favoring $w_0w_a$CDM). BIC approximates the posterior model probability (ambiguous). The Bayes factor asks which model is more probable given the prior (favoring \LCDM{} with broad priors). E-values ask whether the evidence against $H_0$ is calibrated (methodology-dependent). A robust conclusion requires convergence across multiple approaches, which is not yet observed here.

\subsection{The Correct Frequentist Significance}
\label{sec:sigma}

For clarity, we note the proper frequentist conversion of the $\Delta\chi^2 = 11.9$ statistic. The $w_0w_a$CDM model introduces $k = 2$ extra parameters, so under the null hypothesis (\LCDM), $\Delta\chi^2$ follows a $\chi^2$ distribution with 2 degrees of freedom. The $p$-value is:
\begin{equation}
...
The alternative formula $\sigma = \sqrt{\Delta\chi^2} = \sqrt{11.9} \approx 3.5\sigma$ is valid only for $k = 1$ extra parameter, where $\Delta\chi^2 \sim \chi^2(1)$ and the square root maps directly to a standard normal deviate. For $k = 2$, this formula overestimates the significance by approximately $0.7\sigma$.
...
\begin{enumerate}
    ...
    \item \textbf{Different form of prior dependence:} Data-split e-values do not require an explicit prior distribution over $(w_0, w_a)$. However, they are not prior-free: using the point MLE from the training set is effectively equivalent to a point-mass prior centered at $\hat{\theta}$, and the result depends on the split choice, which determines both which data inform $\hat{\theta}$ and which data are available for testing. The uniform mixture e-value explicitly integrates over a prior grid and is transparent about this dependence.
...
\begin{enumerate}
    \item \textbf{Reduced power:} Data-splitting uses only part of the data for testing, reducing statistical power. As discussed in Section~\ref{sec:datasplit}, this power loss is particularly acute for the $w_a$ parameter when splitting by redshift, because the low-redshift training set has limited leverage on time evolution. Our simulation-based power estimate of $E \gtrsim 50$ may be optimistic if the signal resides primarily in $w_a$.
...
\begin{enumerate}
    ...
    \item \textbf{The uniform mixture e-value ($E \approx 15$) constitutes moderate evidence} for $w_0w_a$CDM over \LCDM{} within the DESI BAO data. On the Jeffreys scale, $\ln E \approx 2.7$ falls in the ``strong'' category. This is a valid e-value and should not be dismissed.

    \item \textbf{The data-split e-value ($E = 1.4$) is inconclusive}, not negative. As discussed in Section~\ref{sec:datasplit}, the redshift-based split has limited power for $w_a$, so this result cannot distinguish between ``the signal is absent'' and ``the test lacks power to detect it.'' The roughly 10-fold difference between the mixture and data-split e-values is largely explicable by this power loss.

    \item \textbf{The proper frequentist significance is $\approx 2.8\sigma$}, not $3.5\sigma$. The latter uses $\sqrt{\Delta\chi^2}$, which is valid only for 1 degree of freedom; the correct $\chi^2(2)$ calculation gives $p = 0.0026$ (see Section~\ref{sec:sigma}).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Moderate evidence from mixture e-values.} The uniform mixture e-value yields $E \approx 15$ ($\ln E \approx 2.7$), constituting moderate-to-strong evidence for $w_0w_a$CDM within the DESI BAO data alone.

    \item \textbf{Inconclusive data-split test.} The data-split e-value of $E = 1.4$ is inconclusive rather than negative: the redshift-based split has limited power for $w_a$ because the low-redshift training set provides weak leverage on the time-evolution parameter.

    \item \textbf{The proper frequentist significance is $\approx 2.8\sigma$.} For $\Delta\chi^2 = 11.9$ with $k = 2$ d.o.f., $p = 0.0026$, not the $3.5\sigma$ ($p = 0.0005$) obtained from the incorrect 1-d.o.f.\ formula $\sqrt{\Delta\chi^2}$.
\end{enumerate}

These results, together with independent Bayesian \citep{OngBayesian2025} and tension \citep{WangMota2025} analyses, paint a picture in which the evidence for dynamical dark energy is real but not yet compelling. The path forward requires resolving the inter-dataset tensions---particularly the discrepancy between DES-Y5 and other probes---rather than accumulating further significance within any single statistical framework. Future data from DESI DR3+, Euclid, and the Roman Space Telescope will be essential for determining whether the current hints reflect new physics or systematic effects.
