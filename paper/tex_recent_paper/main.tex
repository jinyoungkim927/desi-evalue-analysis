...
An \textbf{e-value} is a non-negative random variable $E$ satisfying $\E[E \mid H_0] \leq 1$ under the null hypothesis. The concept has deep roots in the work of Ville (1939) on martingale theory, but has been rediscovered and systematized in recent years by \citet{Vovk2021}, \citet{Shafer2021}, and \citet{Ramdas2023} as a unifying framework for hypothesis testing. For cosmologists familiar with Bayesian and frequentist methods, e-values can be understood as follows: like a Bayes factor, an e-value quantifies relative evidence between hypotheses; like a $p$-value, it provides Type~I error control (via Ville's inequality rather than tail probabilities). Unlike Bayes factors, e-values do not require a full prior distribution---a mixture over a discrete grid suffices. Unlike $p$-values, e-values can be multiplied across independent datasets (enabling sequential analysis) and remain valid under optional stopping.
...
\subsection{Uniformly Most Powerful (UMP) E-Values}
\label{sec:ump_theory}

The methods above---data splitting, uniform mixtures, and LOO averaging---all sacrifice power in different ways: data splitting discards test data, mixtures dilute across a grid, and LOO averaging divides by $K$. A natural question is whether one can construct an e-value that uses all the data and avoids this dilution while remaining valid.

For a \textit{simple} null $H_0$ (such as \LCDM, where $w = -1$ exactly), the answer is yes. A likelihood ratio $E = L(\text{data} \mid \theta_1) / L(\text{data} \mid H_0)$ for any pre-specified point alternative $\theta_1$ is a valid e-value by Proposition~\ref{prop:lr}. The key insight, due to \citet{GrunwaldSafeTesting2024}, is that one can choose $\theta_1$ to maximize statistical power without compromising validity---provided $\theta_1$ is determined \textit{before} examining the real data.

Concretely, fix a significance level $\alpha$ (e.g., $\alpha = 0.01$). For each candidate $\theta_1 = (w_0, w_a)$, the rejection region is $\{x : L(x \mid \theta_1)/L(x \mid H_0) \geq 1/\alpha\}$. The \textbf{UMP e-value} uses the $\theta_1^*$ for which this rejection region has the highest power---i.e., the highest probability of rejection when the alternative is true. In an exponential family with a simple null, such a $\theta_1^*$ exists and its rejection region is a superset of the rejection region for any other choice of $\theta_1$ \citep{GrunwaldSafeTesting2024}.

We find $\theta_1^*$ by simulation: for a grid of candidate $\theta_1$ values, we generate synthetic datasets under several plausible alternatives, compute the rejection rate at threshold $1/\alpha$, and select the $\theta_1$ with the highest minimum power across alternatives (a maximin criterion). The resulting $\theta_1^*$ is then fixed, and the likelihood ratio on the real data is computed. Because $\theta_1^*$ depends only on the covariance structure and the simulation design---not on the observed data---the result is a valid e-value regardless of $\alpha$.
...
We use the official DESI DR2 BAO measurements from the CobayaSampler repository,\footnote{\url{https://github.com/CobayaSampler/bao_data}} comprising 13 measurements across 7 redshift bins ($z_{\text{eff}} = 0.295$ to $z_{\text{eff}} = 2.33$). The tracers are BGS ($z_{\text{eff}} = 0.295$, $D_V/r_d$), LRG1 ($z_{\text{eff}} = 0.510$), LRG2 ($z_{\text{eff}} = 0.706$), LRG3+ELG1 ($z_{\text{eff}} = 0.934$), ELG2 ($z_{\text{eff}} = 1.317$), QSO ($z_{\text{eff}} = 1.491$), and Ly$\alpha$ ($z_{\text{eff}} = 2.330$), where the last six each provide both $D_M/r_d$ and $D_H/r_d$. The full $13\times 13$ covariance matrix is block-diagonal by redshift bin: the off-diagonal blocks between different bins are zero, while within each bin the $D_M/r_d$ and $D_H/r_d$ measurements are correlated (with correlation coefficients ranging from $-0.4$ to $-0.5$). All values are taken directly from arXiv:2503.14738, Table~IV \citep{DESI2025}.
...
The dataset tension itself was identified by these authors; our contribution is the cross-prediction e-value framework, which provides a single calibrated number quantifying how well one experiment's preferred dark energy model predicts another's data. The e-value of $E = 0.19$ directly measures how much worse DES-Y5's preferred model performs on DESI data compared to \LCDM---not merely a parameter discrepancy, but a predictive failure.
...
\subsection{UMP E-Value}
\label{sec:ump}

Following the approach described in Section~\ref{sec:ump_theory}, we construct a UMP e-value for the DESI data. We search over a $15 \times 15$ grid of candidate point alternatives $(w_0, w_a) \in [-1.5, -0.5] \times [-2.0, 1.0]$, evaluating power via 500 Monte Carlo simulations under four representative true alternatives: the DESI BAO best-fit $(-0.75, -1.05)$, a milder evolution $(-0.85, -0.50)$, a DES-Y5-like model $(-0.65, -1.20)$, and a Pantheon+-like model $(-0.90, -0.20)$. We use $\alpha = 0.01$ (threshold $1/\alpha = 100$) and refine the grid optimum via Nelder-Mead optimization with 1000 simulations per evaluation.

The optimal point alternative is $\theta_1^* = (w_0^* = -0.87,\, w_a^* = -0.45)$, with minimum power 72\% and average power 84\% across the four alternatives. Evaluating the simple likelihood ratio on the real DESI data:
\begin{equation}
E_{\text{UMP}} = \frac{L(\text{data} \mid w_0^*, w_a^*)}{L(\text{data} \mid \text{\LCDM})} \approx 2180 \quad (\Delta\chi^2 \approx 15.4, \;\; \ln E \approx 7.7)
\label{eq:ump}
\end{equation}

\paragraph{Why is this so much larger than $E_{\text{mix}} \approx 15$?} The uniform mixture averages over 100 grid points, most of which fit the data poorly---this is the Occam penalty. The UMP e-value concentrates all its ``bet'' on a single pre-specified point $\theta_1^*$ that happens to be close to the data's best fit. It uses all 13 measurements (no data splitting) and pays no averaging penalty. The trade-off is that the UMP e-value is optimized for a specific class of alternatives; if the true dark energy dynamics were very different from the alternatives used in the simulation design, a different $\theta_1$ might have higher power.

\paragraph{Validity.} Because $\theta_1^*$ is determined entirely by the simulation procedure (which depends on the covariance matrix and the choice of true alternatives, but not on the observed data), the resulting likelihood ratio is a valid e-value: $\E[E_{\text{UMP}} \mid H_0] = 1$ exactly (Proposition~\ref{prop:lr}). We verify this empirically: under 500 simulations with \LCDM{} as the true model, $\hat{\E}[E_{\text{UMP}}] = 0.62$ and $\hat{P}(E_{\text{UMP}} \geq 100) = 0.000$, confirming calibration.

\paragraph{Interpretation.} The UMP e-value of $\approx 2180$ ($\approx 3.9\sigma$ equivalent) represents the strongest valid e-value we can construct for the DESI data. It demonstrates that the gap between the invalid maximized LR ($\approx 3300$) and the valid mixture e-value ($\approx 15$) is largely due to the Occam penalty inherent in averaging, not due to a fundamental weakness of the evidence. When the alternative is concentrated at a well-chosen point rather than spread across a grid, most of the frequentist signal is recoverable as a valid e-value.
...
\begin{enumerate}
    ...
    \item \textbf{Convergence of independent methods:} The uniform mixture ($E \approx 15$), LOO average ($E \approx 10$), and UMP ($E \approx 2180$) are constructed differently but all constitute valid evidence against \LCDM. The range from $E \approx 10$ to $E \approx 2180$ quantifies the cost of Occam penalization: spreading probability across a grid versus concentrating on a single point.
...
\begin{enumerate}
    ...
    \item \textbf{Simplified cosmology and no marginalization over nuisance parameters:} We fix the background cosmological parameters $\{h, \Omega_m, r_d\}$ to Planck 2018 values and use approximate distance calculations rather than full Boltzmann codes (CAMB/CLASS). In a full treatment, one would marginalize over these parameters, which could affect the e-values in two ways: (i) broadening the effective alternative hypothesis would reduce the uniform mixture e-value (more ``wasted'' prior volume), while (ii) allowing the fit to adjust $\Omega_m$ and $h$ could improve the $\chi^2$ and increase it. These effects partially cancel, but the net impact is unknown without explicit computation. This limitation is shared with any analysis that conditions on external parameter constraints, including standard $\Delta\chi^2$ and BIC calculations.

    \item \textbf{Point estimates for SNe:} We use published best-fit values for supernova constraints rather than full posteriors. Using the full posterior would be more rigorous---for example, marginalizing the cross-dataset e-value over the supernova $(w_0, w_a)$ posterior would account for parameter uncertainty in the training dataset.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Valid e-value methods span a wide range depending on how the alternative is specified.} The uniform mixture ($E \approx 15$), LOO average ($E \approx 10$), and UMP e-value ($E \approx 2180$) are all valid. The mixture and LOO methods pay an Occam penalty by averaging over parameter space or folds, yielding moderate evidence ($E \approx 10$--$15$, $\approx 2.2$--$2.4\sigma$). The UMP e-value, which concentrates its bet on a single pre-specified point alternative optimized for power, recovers most of the frequentist signal ($\approx 3.9\sigma$). The choice between these reflects how conservative one wishes to be: averaging is more robust to misspecification of the alternative, while the UMP approach is more powerful when the alternative is well-characterized.
    ...
    \item \textbf{DESI's BAO-only significance is $\approx 3.0\sigma$}, correctly computed via the $\chi^2(2)$ distribution and Wilks' theorem. Our e-value analysis finds that approximately $0.7\sigma$ of this is absorbed by the Occam penalty when averaging over the $(w_0, w_a)$ parameter space, yielding the moderate evidence of $E \approx 10$--$15$ ($\approx 2.2$--$2.4\sigma$ equivalent). A note on this sigma conversion: we use $\sigma \approx \sqrt{2\ln E}$, which maps the e-value to the significance level of a one-parameter likelihood ratio via $E = e^{\sigma^2/2}$. This is a convenient scale for comparison with frequentist results, but it is approximate---e-values and $p$-values answer fundamentally different questions, and there is no exact universal conversion between them. The mapping is most accurate when the e-value arises from a simple likelihood ratio; for mixture or LOO average e-values, it should be understood as a rough calibration aid rather than an exact equivalence.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Valid e-value methods span from moderate to strong evidence.} The uniform mixture ($E \approx 15$) and LOO average ($E \approx 10$) pay an Occam penalty by averaging over parameter space, yielding moderate evidence ($\approx 2.2$--$2.4\sigma$). The UMP e-value ($E \approx 2180$, $\approx 3.9\sigma$), which concentrates on a single pre-specified point alternative optimized for power, recovers most of the frequentist signal as calibrated evidence. The gap between $E \approx 15$ and $E \approx 2180$ quantifies the cost of averaging: roughly two orders of magnitude, or $\approx 1.5\sigma$.
...
These results, together with independent Bayesian \citep{OngBayesian2025} and tension \citep{WangMota2025} analyses, paint a nuanced picture. Depending on how one specifies the alternative, valid e-values range from $E \approx 10$--$15$ (averaging methods) to $E \approx 2180$ (UMP)---real evidence that should be taken seriously. The most informative finding remains the cross-dataset tension: until the discrepancy between DES-Y5 and other probes is understood, the possibility that $w_0w_a$CDM is absorbing inter-dataset tension rather than detecting new physics cannot be ruled out. Future data from DESI DR3+, Euclid, and the Roman Space Telescope will be essential for determining whether the current hints reflect new physics or systematic effects.
